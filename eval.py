...
GOLDEN = [
    ...
    ("soldier",   "מה זה ריתוק משקי ומתי אפשר להטיל אותו?", "50.0405"),
    ...
    ("soldier",   "איך מגישים בקשת חנינה על פסק דין של בית דין צבאי?", "33.0314"),
    ...
    ("reserve",   "כמה נקודות זכות מקבל חייל מילואים על 20 ימי שירות בשנת 2016?", "3.0110"),
    ("reserve",   "עד מתי אפשר להגיש השגה על חישוב התגמול הנוסף למילואים?", "3.0110"),
    ...
    ("commander", "האם יום גיבוש ליחידה יורד לחיילים מימי החופשה?", "35.0408"),
    ...
    ("commander", "איזה מענק מקבל משרת קבע בסיום שירותו ומה נחשב לשירות תקין לחישובו?", "3.0502"),
...
DIRTY = [
    ...
    ("reserve",   "כמה כסף מקבלים על מילואים חוץ מהמשכורת?", ("3.0110", "35.0206", "35.0209")),
...
FACTS = [
    ...
    ("soldier", "מסוכן באזור המגורים שלי — מי מוסמך לאשר לי לשאת נשק הביתה ולכמה זמן האישור תקף?", "2.0101",
     [["אל\"ם"], ["חצי שנה", "שישה חודשים", "6 חודשים"], ["2.0101", "נשיאת נשק"]]),
    ("soldier", "יום כיף יחידתי יורד לי מימי החופשה השנתית?", "35.0408",
     [["ימי פעילות", "לא יימנו", "לא יורד", "אינם יורדים", "לא נמנים"], ["35.0408", "גיבוש"]]),
    ...
    ("reserve", "עשיתי 60 ימי מילואים בצו 8 — כמה כסף מגיע לי בשובר הנופש?", "שוברי-נופש-מילואים",
     [["4,500", "4500"], ["לוחם"], ["ילד"]]),
    # pilot 2026-07-26 ("לאיים במשפט"): compound victim-commander question — the
    # verdict must rule on HIS act (אסור/עבירה per חש"צ 62), not open with the
...
def run_structural() -> int:
    ...
    cc_cases = [
        ...
        ("זכאי לתשלום מלא", ("yes", "✓")),
        ("לא נמצאה בפקודות חובה למצוא מחליף מיידי", ("none", "ⓘ")),
        ("מוסמך", None),                          # bare authority — answers nothing
