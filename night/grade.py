...
קודם כול פרק את השאלה לחלקים הענייניים שלה, וספור:
- `answered_parts` — על כמה מהם התשובה נתנה מענה ממשי מתוך הפקודות.
- `unanswered_parts` — על כמה היא לא נתנה מענה, מכל סיבה.
...
⚠ אל תעדיף רמה כלשהי. הריצה הראשונה החזירה 76 `led_known`, 24 `full`, ואפס
`partial` ואפס `refused` — כי הפרומפט הקודם כתב ליד `led_known` שהיא „הצלחה,
לא כישלון" והמדרג פשוט הסכים. הספירה קודמת לסיווג בדיוק כדי למנוע את זה."""
