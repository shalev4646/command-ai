...
try:
    ...
    def _verdict_clauses(_content):
        return []
# entitlements is a deterministic, order-cited lookup (no LLM). Same defensive
# import as the other sibling modules: a stale cached cloud build pairing a new
# app.py with an older tree just hides the tool instead of crashing the boot.
try:
    import entitlements
except Exception:
    entitlements = None
...
with st.sidebar:
    ...
    if LETTER_TYPES and st.button("📄 מחולל מכתבים", key="open_letters", use_container_width=True):
        _letters_dialog()
    # deterministic entitlement lookup (leave days + subsistence/family
    # payments) — no quota, no LLM call; hidden if the module didn't import
